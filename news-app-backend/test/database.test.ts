...
describe("Database Operations", () => {
  ...
  it("should add summaries to the database, retrieve them and check for their existence", async () => {
    ...
    const result: NewsSummariesRecord = (await addSummaries(
      day,
      summaries,
      urls,
      category,
      numNews,
    )) as NewsSummariesRecord;

    expect(result.id).toEqual(
      new RecordId(
        `summaries-${category}`,
        `${day.format("YYYY-MM-DD")}-${numNews}`,
      ),
    );
    expect(result.summaries).toEqual(summaries);
    expect(result.urls).toEqual(urls);

    const result2: NewsSummariesRecord = (await getSummaries(
      day,
      category,
      numNews,
    )) as NewsSummariesRecord;
