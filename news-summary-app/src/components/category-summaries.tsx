...
export default function CategorySummaries() {
  ...
  useEffect(() => {
    ...
    const previousDayNewsFetch = fetch(
      `${import.meta.env.VITE_NEWS_ENDPOINT}/${category}`,
      {
        method: "POST",
        headers: {
    ...
    const currentDayNewsFetch = fetch(
      `${import.meta.env.VITE_NEWS_ENDPOINT}/${category}`,
      {
        method: "POST",
        headers: {
    ...
    Promise.all([previousDayNewsFetch, currentDayNewsFetch])
      .then(([previousDayNewsRes, currentDayNewsRes]) => {
        ...
        if (previousDayNewsRes.ok && currentDayNewsRes.ok) {
          return Promise.all([
            previousDayNewsRes.json(),
            currentDayNewsRes.json(),
          ]);
        } else {
      ...
      .then(([previousDayNews, currentDayNews]) => {
        ...
        const allUrls = [
          ...currentDaySummariesUrls,
          ...previousDaySummariesUrls,
        ];
  ...
  return (
    <main className={styles["summaries-body"]}>
      <div className={styles["summaries-header"]}>
        ...
        >
          <img src={nextDayBtn} alt="Next Day" />
        </button>
      </div>
