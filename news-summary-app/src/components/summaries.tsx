...
export default function Summaries() {
  ...
  const newsCategoriesDescription: string[] = [
    "Top daily stories",
    "Global news updates",
    ...
    "Wellness and medicne",
  ];

  const newsOptions: Record<NewsCategory, string> = Object.fromEntries(
    newsCategories.map((category, i): [NewsCategory, string] => [
      category,
      newsCategoriesDescription[i],
    ]),
      ...
      <div className={styles["user-header"]}>
        ...
        <button
          ...
          onClick={() =>
            logout({
              logoutParams: {
                returnTo: `http://localhost:${import.meta.env.VITE_PORT}`,
              },
            })
          }
        >
          ...
          {Object.entries(newsOptions).map(([category, description]) => {
            return (
              <li key={category} className={styles["news-category"]}>
                <Link
                  to={`/summaries/:${category.toLowerCase()}`}
                  className={styles["news-category-link"]}
                >
                  <img
                    className={styles["news-category-img"]}
                  ...
                  <div className={styles["news-category-detials"]}>
                    ...
                    <h4 className={styles["news-category-description"]}>
                      {description}
                    </h4>
                  </div>
                </Link>
